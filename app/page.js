...
const roles = [
  { id: 'citizen', name: 'Citizen', desc: 'Raise an issue and follow every update.', icon: 'C', phone: '+918282909044', tint: 'coral' },
  { id: 'admin', name: 'Admin', desc: 'Route demand across the civic network.', icon: 'A', phone: '+919999000023', tint: 'blue' },
  { id: 'supervisor', name: 'Supervisor', desc: 'Prioritise work and keep crews moving.', icon: 'S', phone: '+919999000004', tint: 'amber' },
  { id: 'worker', name: 'Worker', desc: 'See your queue, evidence and completed work.', icon: 'W', phone: '+919999000005', tint: 'teal' },
];
...
export default function LoginPage() {
  ...
  return <main className="v-login">
    ...
    <section className="v-login-panel"><div className="v-login-kicker">KOPARGAON MUNICIPAL COUNCIL <span>/</span> SECURE DEMO</div><h1>Choose your<br /><em>workspace.</em></h1><p className="v-login-lede">Start where you belong. You can move through the entire service journey with one simple sign-in.</p>
      {step === 'role' ? <>
        <div className="v-role-grid">{roles.map(item => <button type="button" key={item.id} className={`v-role-card ${selected === item.id ? 'is-selected' : ''}`} onClick={() => chooseRole(item)}><span className={`v-role-icon v-role-${item.tint}`}>{item.icon}</span><span><strong>{item.name}</strong><small>{item.desc}</small></span><b>Open</b></button>)}</div>
      ...
      </> : <form className="v-login-form" onSubmit={verifyOtp}><div className="v-otp-note"><span className="v-role-icon v-role-teal">Code</span><div><strong>Code sent to {phone}</strong><small>Use 123456 in the demo environment.</small></div></div><label>One-time code<input value={otp} onChange={e => setOtp(e.target.value.replace(/\D/g, '').slice(0, 6))} placeholder="123456" inputMode="numeric" autoFocus /></label><button className="v-button v-button-primary v-button-wide" disabled={busy}>{busy ? 'Opening...' : 'Open workspace'}</button><button type="button" className="v-demo-link" onClick={() => setStep('role')}>Change workspace</button></form>}
