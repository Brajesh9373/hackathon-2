...
function buildRoleSystemPrompt({ designation, purpose }) {
  ...
  return [
    ...
    normalized === 'Citizen' ? 'For resolution verification, ask one direct question: is the reported issue fully fixed? Treat only a clear yes as confirmation. A no, uncertainty, silence, or unrelated answer must keep the case open.' : 'Do not request passwords, OTPs, payment details, or sensitive personal information.',
  ].join(' ');
}
